...
// Fungsi untuk rotasi kiri-kanan (Left-Right Case / Zig-Zag)
AVLNode* _leftRightRotate(AVLNode* node) {
    // Langkah pertama: rotasi kiri pada anak kiri
    node->left = _leftRotate(node->left);
    
    // Langkah kedua: rotasi kanan pada node
...
AVLNode* _insert(AVLNode* node, int data) {
    ...
    // Kasus 1: Rotasi Kiri pada anak kiri dan rotasi kanan pada node (Left-Right Case)
...
int main() {
    ...
    avl_insert(&avl, 30);
    avl_insert(&avl, 10);  // Ini akan menyebabkan rotasi kiri pada anak kiri
    avl_insert(&avl, 20);  // Ini akan menyebabkan rotasi kanan pada node
