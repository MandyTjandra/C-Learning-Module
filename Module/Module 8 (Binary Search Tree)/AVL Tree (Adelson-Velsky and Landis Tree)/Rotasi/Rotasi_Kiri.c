...
AVLNode* _insert(AVLNode* node, int data) {
    ...
    // Kasus 1: Rotasi Kiri Kanan (Right-Right Case)
    if (balance < -1 && data > node->right->data)
        return _leftRotate(node);

    // Kasus lainnya seperti rotasi kanan-kiri dan kiri-kiri bisa ditambahkan
...
int main() {
    ...
    avl_insert(&avl, 10);
    avl_insert(&avl, 20);
    avl_insert(&avl, 30);  // Ini akan menyebabkan rotasi kiri (Right-Skewed Case)
